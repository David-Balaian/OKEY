...
export default function Game() {
    ...
    function handleMouseUp() {
        if(selectedDomino){
            ...
            if(dropLocation || dropLocation === 0){
                dispatch(DROP(selectedDomino.i, dropLocation))
    ...
    return (
        <div className={styles.container} onMouseUp={handleMouseUp} onMouseMove={handleDrag} >
            <div className={styles.gameTable} >
                ...
                <div className={styles.dominoContainerBorder} style={{zIndex:selectedDomino ? 1001 : "unset", bottom: "41%", right: "20px" }} onMouseOver={()=>{ if(!selectedDomino) return; dispatch(DROP_LOCATION("userSide"))}} onMouseLeave={()=>{if(!selectedDomino) return; dispatch(DROP_LOCATION(null))}}  > <div className={styles.dominoContainer} style={{height:"100%", width:"100%"}} > {userssDomino && <Domino item={userssDomino} />}</div> </div>
                <div className={`area ${styles.dominosContainer}`} ref={dominosContainerRef} >
                    {user.dominos.map((item, i) => {
                        if(item){
                            ...
                            return <div key={i} style={selectedDomino ? {zIndex:1001} : {}} onMouseOver={()=>{ if(!selectedDomino) return; dispatch(DROP_LOCATION(i))}} onMouseLeave={()=>{if(!selectedDomino) return; dispatch(DROP_LOCATION(null))}} className={`${item ? "" : "dropable"} ${styles.singleDominoContiner}`}/>
                        }
                        // return <> {!item ? 
...
function Domino({ item, i }) {
    ...
    return (
        <animated.div
            ref={dominoRef}
            style={{...props, position:"absolute", top: i >= 13 ? "50%" : "0px", left:`calc((100% / 13) * ${i >= 13 ? i - 13 : i})`, }}
            className={styles.domino} 
            onMouseDown={() => dispatch(SELECT_DOMINO({ item, i, ref:dominoRef, styleSetter: setter }))} 
        >
